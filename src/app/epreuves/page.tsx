...
export const metadata: Metadata = {
  title: "Épreuves scolaires togolaises — BAC, BEPC, BTS, CEPD",
  description:
    "Accédez gratuitement aux sujets et corrigés des examens nationaux togolais " +
...
export default async function PageEpreuvesLanding() {
  ...
  // Grouper les examens par examen spécifique (BAC, BEPC, CEPD, BTS)
  const bacs = examens.filter((e) => e.examen?.toUpperCase() === "BAC");
  const bepcs = examens.filter((e) => e.examen?.toUpperCase() === "BEPC");
  const cepds = examens.filter((e) => e.examen?.toUpperCase() === "CEPD");
    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      <header className="mt-6 max-w-3xl">
        <h1 className="font-serif text-4xl font-semibold leading-tight tracking-tight text-stone-900 dark:text-zinc-100 sm:text-5xl">
          Épreuves et Examens Scolaires.
        </h1>
        <p className="mt-4 text-base leading-relaxed text-stone-600 dark:text-zinc-400">
          Révisez et préparez vos examens officiels du Togo avec notre bibliothèque interactive. 
          Retrouvez les sujets originaux et leurs corrigés détaillés pour le CEPD, le BEPC, le BAC et le BTS.
        </p>
      </header>

      {/* Raccourcis examens nationaux */}
      ...
      <div className="mt-14 grid grid-cols-1 gap-10 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <section>
            <div className="flex items-center justify-between mb-6 border-b border-stone-100 dark:border-zinc-800 pb-3">
              <h2 className="font-serif text-2xl font-semibold text-stone-900 dark:text-zinc-100">
                Examens Officiels
              </h2>
              <span className="text-xs text-stone-500 dark:text-zinc-400 font-medium bg-stone-100 dark:bg-zinc-900 px-2 py-1 rounded-md">
                {examens.length} épreuves
              </span>
            </div>

            {examens.length === 0 ? (
              <p className="text-sm text-stone-500 dark:text-zinc-400 italic">Aucun examen national répertorié pour le moment.</p>
            ) : (
              <div className="space-y-6">
                {/* On regroupe par année */}
                {Array.from(new Set(examens.map(e => e.annee)))
                  .sort((a, b) => b - a)
                  .map(annee => (
                    <YearGroup 
                      key={`exam-group-${annee}`} 
                      annee={annee} 
                      docs={examens.filter(e => e.annee === annee)} 
                    />
                  ))
                }
              </div>
...
function ExamTypeCard({ exam, titre, count, color, badgeColor }: { 
  exam: string; 
  titre: string; 
  count: number; 
  color: string;
  ...
  return (
    <div className={`bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-800 rounded-xl p-4 cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md ${color} group`}>
      ...
      <p className="text-[11px] text-stone-500 dark:text-zinc-400 mt-1">
        {count} {count > 1 ? "épreuves disponibles" : "épreuve disponible"}
      </p>
    </div>
  );
}
