      ...
      <article className="mt-6">
        ...
        <header className="border-b border-zinc-200 pb-6 mb-8">
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <span className="bg-emerald-50 text-emerald-700 border border-emerald-100 text-xs px-2.5 py-1 rounded-full font-semibold">
          ...
          <h1 className="font-serif text-3xl font-bold tracking-tight text-stone-900 sm:text-4xl leading-tight">
            {doc.type === "examen"
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4 text-xs text-zinc-400">
            <span>Pays : <strong className="text-zinc-600 capitalize">{doc.pays}</strong></span>
            {doc.duree_minutes && (
              <>
                <span>•</span>
                <span>Durée : <strong className="text-zinc-600">{Math.floor(doc.duree_minutes / 60)}h{doc.duree_minutes % 60 > 0 ? doc.duree_minutes % 60 : ""}</strong></span>
              </>
            )}
            {doc.coefficient && (
              <>
                <span>•</span>
                <span>Coefficient : <strong className="text-zinc-600">{doc.coefficient}</strong></span>
              </>
            )}
            <span>•</span>
        ...
        <section className="bg-zinc-50 border border-zinc-150 rounded-2xl p-6 mb-8 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Bouton Sujet */}
            <div className="border border-zinc-200 bg-white rounded-xl p-4 flex flex-col justify-between">
              <div>
                <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wide">
                  Document de travail
                </span>
                <h3 className="font-bold text-sm text-stone-800 mt-0.5">
                  Sujet de l&apos;épreuve
                </h3>
              </div>
              <div className="mt-4">
                {doc.fichiers.sujet ? (
                  ...
                  <button
                    disabled
                    className="w-full bg-zinc-100 text-zinc-400 text-xs font-medium px-4 py-2.5 rounded-lg cursor-not-allowed"
                  >
        ...
        {/* Visualisation directe de l'épreuve (Sujet) */}
        {doc.fichiers.sujet && (
          <section className="mt-10 block">
            <h2 className="font-serif text-2xl font-bold text-stone-900 mb-6 pb-2 border-b border-zinc-100">
              Visualisation du Sujet
            </h2>
            
            <div className="bg-white border border-zinc-200 rounded-2xl p-4 shadow-sm overflow-hidden flex justify-center items-center">
              {doc.fichiers.sujet.toLowerCase().endsWith(".pdf") ? (
                <iframe
                  src={`${doc.fichiers.sujet}#toolbar=0`}
                  width="100%"
                  className="rounded-xl border border-zinc-150 w-full h-[500px] md:h-[800px]"
                ...
                <img
                  ...
                  className="max-w-full h-auto rounded-xl border border-zinc-150 shadow-sm"
                />
              )}
            </div>
          </section>
